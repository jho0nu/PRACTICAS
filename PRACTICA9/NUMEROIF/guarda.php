	...
	<body>
		<div class="contenedor">
				<div class="font-titulo">
					<center>NUMERO</center></div>
<?php

	$num=$_POST['numero1'];
	
	if ($num==25 || $num==30 || $num==75) {
		$cu=pow($num,1/3);
		echo '<div class="form-input ap">
					<label for="ap"><i>LA RAIZ CUBICA DE </i>'.$num.'<i> ES </i>'.$cu.'</label>
				</div>';
	}
	else if ($num==10 || $num>=100 || $num>=3 && $num<=8 || $num>=77 && $num<=90) {
		$div=$num/10;
		echo '<div class="form-input ap">
					<label for="ap"><i>EL NUMERO DIVIDO  </i>'.$num.'<i> ENTRE 10 ES IGUAL A </i>'.$div.'</label>
				</div>';
	}
	else if ($num<50) {
		$cua=pow($num,2);
		echo '<div class="form-input ap">
					<label for="ap"><i>EL CUADRADO </i>'.$num.'<i> ES </i>'.$cua.'</label>
				</div>';
	}
	else {
		echo '<div class="form-input ap">
					<label for="ap"><i>EL NUMERO </i>'.$num.'<i> NO CUMPLE CON NADA </i></label>
				</div>';
	}
?>
	</div>
